...
In this task you will learn how to edit the configuration of Azure Arc enabled SQL Managed Instances with the Azure Data CLI

1. On your LabVM launch a **Command Prompt** window (Select search on the task bar, type cmd, and select Enter).

1. In the command prompt, run the following command to see configuration options.

   ```BASH
   azdata arc sql mi edit --help
   ```
1. You can set the cpu core and memory requests and limits by running the following command. Make sure to replace the **NAME_OF_SQL_MI** with your SQL Managed Instance name.

   ```BASH
   azdata arc sql mi edit --cores-limit 4 --cores-request 2 --memory-limit 4Gi --memory-request 2Gi -n <NAME_OF_SQL_MI>
   ```

1. Now run the following command to view the changes made to the SQL managed instance.
