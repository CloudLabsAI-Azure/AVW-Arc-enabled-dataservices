...
In this exercise, you will create an **Azure SQL managed instance - Azure Arc** and restore and migrate the database using multiple methods. 

Also, you will be exploring the Kibana and Grafana Dashboards and upload the logs and metrics to the Azure portal and view the logs.
...
1. Now in the **SQL managed instance - Azure Arc Data Controller dashboard**, copy the **IP Address** from external endpoint value. you will also see that the status is **Ready**.
...
1. Enter the following in the connection details page and click on **Connect**.
   ...
   >**Note**: Make sure you have entered **IP Address** only and avoid **port number** 
...
## Task 5: Migrate: Migrate and Restore SQL Server DB to Azure Arc enabled SQL Managed Instance from Blob storage and Azure arc pod.
   ...
   In this Task, you will be migrating and restoring the SQL server database to the Azure arc enabled SQLMI. 
   
   There are two methods to do the migration and restore - one is using the Azure blob storage to back up and restore DB to SQLMI and the second one is using the Kubectl commands. 
 ...
 **Method 2: Copy the backup file into an Azure SQL Managed Instance - Azure Arc pod using kubectl**
   ...
   4. Run the below query to restore the database to the SQL Managed instance - Azure arc
      ...
      GO
      ```
   5. Now you see that the database is created
...
## Task 7: Monitor with Azure Data Studio
  ...
  1. Open Azure Data studio on the provided Virtual Machine on the left and right-click on the SQL MI resource under the Azure Arc controller and click on manage.
  ...
  1. Now go back to the Azure Data studio which you opened earlier on on the provided Virtual Machine on the left.
  ...
  1. You can explore the page for Grafana. 
      ...
      > ***Info***:  You can learn more about Grafana here: https://docs.microsoft.com/en-us/azure/azure-arc/data/monitor-grafana-kibana
