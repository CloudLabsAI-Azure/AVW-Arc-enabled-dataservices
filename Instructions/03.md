...
1. In the next blade, Accept the Privacy and license terms and click **Select** at the bottom.
...
1. Scroll down to the bottom of the page an you will see that the PostgreSQL Hyperscale - Azure Arc instance creation has started
...
1. Now connect Azure arc enabled Postgres server using above connection string details.

   ![](./images/pg-add-arc-db.png "")

1. Under the connection tab enter the below details from the above connection string details similar to the following one.
...
1. First open Command Prompt shortcut from desktop on your LabVM.

**Verify configuration**

1. Now, you will verify if the Hyperscale server group has been configured to use backup storage class.

1. Run following command to view details of Postgres Hyperscale server
   ```BASH
...
1. You will restore the entire content of a backup.
   ...
   **Restore the server group postgres to a different server group postgres01:**
   ...
   Run following command to restore **postgres** DB onto **postgres01** DB
   ```BASH
...
In this task you will work on migration of on-premise Postgres database to Azure Arc enabled PostgreSQL Hyperscale server group.

- Backup from Source Server:
   You will take back of A Postgres Server running on-premises on a bare metal server named **Postgres**. It hosts a database named **Arc-Demo-PG** that has 4 tables.
...
- Restore to Destination Server:
   You will restore database **Arc-Demo-Bkp** from source server **postgres** on to A Postgres server **postgres01** running in an Azure Arc environment with new DB named **Restored_Arc-Demo-Bkp**. 
...
**Restore the database in your Arc setup**
   ...
   1. Point to the folder that contains the backup file to restore: Arc-Demo-Bkp
...
**Verify that the database was successfully restored in your Azure Arc enabled PostgreSQL Hyperscale server group**
   ...
   Test from psql inside your Azure Arc setup:
   ...
   - Right-click on Database and select Query tool to open a new query window 
   - Use the below query to see the results of **pgbench_accounts** table 

   ```BASH
   select * from Public.pgbench_accounts;
   ```

   - You'll see the data that you restored from the on-premises Postgres instance:
