...
1. The database migration was successful if the row counts are same.


## Task 3: Distribution(Shard) of data on worker nodes in Azure Arc PostgreSQL – Hyperscale 

In this task, you will work on Distribution (Shard) of data on worker nodes.

First, open query tool by right click on Postgres database server and select "Query Tool" option.
...
### Table Rows, shards and placements
1. You have to distribute table data throughout the Hyperscale server group. 
   ...
   Hyperscale (Citus) assigns each row to a shard. Every row will be in exactly one shard, and every shard can contain multiple rows.
...
1. Hyperscale uses the **pg_dist_shard** table to assign rows to shards. You can query to see which values map to which shard IDs.

   Run following query to see which values map to which shard IDs
   ```BASH
   ...
   --------------------------------------
                                 102008
   ```
   ![](./images/pg-shard-query-05.png "")

   You have mapped rows to shards in logical way. But, Shards must be assigned to specific worker nodes for storage, in what Hyperscale calls shard placement.

   You can look at the shard placements in **pg_dist_placement** table.
   
   Run following query to see where each shard is present 
   ```BASH
   select
   ...
   limit 5;
   ```
    > **Note**: limit the output to the first five placements.

   By joining all the 3 table data with the other metadata tables you have seen where each shard is present.

   ```BASH
   table | shard  |    host
   -------+--------+------------
   users | 102008 | arcpostgres
   users | 102009 | arcpostgres
   users | 102010 | arcpostgres
   users | 102011 | arcpostgres
   users | 102012 | arcpostgres
   ```
   ![](./images/pg-shard-query-06.png "")

### Identify skewed distribution   

To run server group most efficiently you have to place data evenly on worker nodes
...
1. Run Following query to create sample data for **users** table
   ...
   102012 | true    | 16 kB
   ```
   ![](./images/pg-shard-query-08.png "")

   You can see the shards are of equal size. 
...
1. Run following query to find the average age of users, 
...
1. Actually in the background the Hyperscale executor creates a separate query for each shard, runs them on the workers, and combines the result. 
    
    To see the details You need to run the **PostgreSQL EXPLAIN** command 
   ...
   Aggregate  (cost=500.00..500.02 rows=1 width=32)
      ...
      ->  Task
         Node: host=arcpostgres-0.arcpostgress.svc.arcdc.svc.cluster.local port=5432 dbname=postgres
         ->  Aggregate  (cost=41.75..41.76 rows=1 width=16)
            ->  Seq Scan on users_102008 users  (cost=0.00..22.70 rows=1270 width=4)
   ```
   ![](./images/pg-shard-query-10.png "")   


   The output shows an example of an execution plan for a *query fragment* running on shard 102040 
