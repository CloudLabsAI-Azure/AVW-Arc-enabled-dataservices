...
**Connect onpremise source database and Azure Arc enabled PostgreSQL server**

1. In the command prompt run the following to get postgres servers list in AKS using the Azure Arc data controller. 
...
2. To get conection string details for postgres01 server endpoint run following command.
   ...
   ```
3. On your LabVM open PgAdmin tool from start menu to connect Azure arc enabled postgres Hyperscale database 
...
5. Under connection tab enter below detail from above connection string details similar to following one.
...
## Task 5: How to migrate from different cloud/server?
