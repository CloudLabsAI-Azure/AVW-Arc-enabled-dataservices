...
In this exercise, you will implement on following concepts,
...
2. Run the following command to get connection string details for postgres01 server endpoint .
...
2. First, you need to verify that Hyperscale server group has been configured to use backup storage class.

   - Run following command to view details of Postgres Hyperscale server
   ```BASH
...
3. Run following command to take a full backup of the entire data and log folders of Postgres server group by :
   ```BASH
...
5. You will restore the entire content of a backup.
