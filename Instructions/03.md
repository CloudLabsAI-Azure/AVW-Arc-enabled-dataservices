...
1. Now, you will click on the arrow icon next to postgreLocal Server and expand
...
1. In the blade that comes up, provide the Database name as **Restored_Arc-Demo-PG** and then click on the Save button at the bottom of the blade.
...
We will verify if the database has been restored from the Database running on the local server to the Arc enabled PostgreSQL Server by querying the data in both databases.
...
1. You have to distribute table data throughout the Hyperscale server group. 
   Distributing a table assigns each row to a logical group called a shard. 
...
1. Open Azure Data studio on Jumpbox and right-click on the Postgres server resource under the Azure Arc controller and click on manage.
...
In this task, you will create a log analytics workspace to upload logs for your Azure Arc enabled SQL managed instances and Azure Arc enabled PostgreSQL Hyperscale server groups to Azure Monitor and view your logs in the Azure portal

1. In the command prompt run the following command to log in to azure. On the sign-in page enter the username and password, you can find the username and password from the environment details tab.
...
1. From the output window copy the **customerID** value and save the value locally in the text file, you will use these values in the latter part of the lab.
