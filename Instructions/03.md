...
In this task, you will learn how to create and connect the Postgres Hyperscale Server group.

1. First, click on **+ New Instance** in the Azure Arc Data Controller dashboard.

1. From the deployment options blade, select the PostgreSQL Hyperscale server group - Azure Arc. 
...
1. Now a **deploy.postgres.existing.arc** Notebook will open after clicking on the deployment button and automatically execute the cells to deploy the PostgreSQL Hyperscale servergroup.
...
1. The deployment will take around 5 to 10 minutes and once afer successful deployment, you can see the output "arcpostgres is Ready"
...
1. Now, select the connection tab and enter the connection string details shown in the previous azdata endpoint list command.
...
You will take a backup of the Postgres Server running on-premises on a bare metal server named **PostgreLocal**. It contains a database named **Arc-Demo-PG** which has 4 tables.

#### Take a backup of the source database in on-premises

1. Now, you will click on the arrow icon next to postgreLocal Server to expand the menu
...
You will restore the sample database **Arc-Demo-Bkp** from source server **PostgresLocal** on to the Postgres server **postgres01** running on an Azure Arc environment with a new Database named **Restored_Arc-Demo-Bkp**. 
...
You will verify if the database has been restored from the Database running on the local server to the Arc enabled PostgreSQL Server by querying the data in both databases.

   Now, you will use PgAdmin in which you are already connected to both Postgres instances ( local and Arc ).
...
In this task, you will work on the Distribution (Shard) of data on worker nodes.

First, open the query tool by right-clicking on Postgres database server and select the "Query Tool" option.

![](./images/pg-query-tool-db.png "")

In the previous Tasks, we created a  Postgres Hyperscale server group with two worker nodes.

1. First open Run following query to check the active workers in the **pg_dist_node table**.
...
1. You have to distribute table data throughout the Hyperscale server group. 
   ...
   Run following queries create a table and distribute it
...
### View distributed data by Querying distributed tables
   ...
   In the previous sections, You have seen how distributed table rows are placed in shards on worker nodes.
   Now you will see the distributed data present in the **users** table.

1. Run the following query to find the average age of users, 

   ```BASH
   select avg(current_date - bday) as avg_days_old from users;
   ```

   treating the distributed **users** table like it's a normal table
...
1. Now, you will verify if the Hyperscale server group has been configured to use a backup storage class.
...
1. You will be Restoring the backup of server group postgres onto itself.
...
1. On your Lab VM launch **Command Prompt** from desktop.
...
In this task, you will create a log analytics workspace to upload logs for your Azure Arc enabled PostgreSQL Hyperscale server groups to Azure Monitor and view your logs in the Azure portal

1. In the command prompt run the following command to log in to azure CLI.
   ...
   > **Note**: If you are redirected to sign-in page enter the username and password which you can find them on the environment details tab.

1. In the command prompt run the following command to create a log analytics workspace. 

    >**Note**: Please make sure to replace the resource group name and workspace name. You can find the resource group name from the environment details tab and for workspace name enter a globally unique value.
...
1. From the output window copy the **customerID** value and save the value locally in the **Notepad** file, you will use these values in the latter part of the lab.
...
1. Now run the following command to retrieve the access keys required to connect to your log analytics workspace. Make sure to replace resource group name and workspace name
   ...
   ```BASH
     az monitor log-analytics workspace get-shared-keys --resource-group <resource group name> --workspace-name <Unique name of workspace>
   ```

   ![](./images/LogWorkspace-Keys.PNG "")
   
1. From the output window copy the **"primarySharedKey** value and save the value in the **Notepad** file, you will use these values in the later part of the lab.

1. In the command prompt, replace the **customerId**, **primarySharedKey** in the following command with the values which you copied earlier and run it.  
   >**Note**: This command saves the customerId and primarySharedKey values in an environment variable to use later in the lab.
...
1. You will have a query in the query editor that will show the most recent 10 events in the log. In the query editor click on **Run** to display the results.
   ...
   > **Note**  Now you have created the **Log Analytics workspaces** but you can't upload any usages or logs. You will be uploading the Logs and usages in the Next exercises.  
