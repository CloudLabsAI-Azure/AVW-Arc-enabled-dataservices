...
Now that you are familiar with the existing Kubernetes cluster and Data controller, let perform the following in this exercise:
...
## Task 2: How to migrate from different Multi-cloud servers?

Now, let's migrate a PostgreSQL database to Azure Arc enabled PostgreSQL Hyperscale server group. The following steps are applicable to PostgreSQL database running on any cloud or on-premises. 

#### **Backup from Source Server:**

You will take a backup of the Postgres Server running on on a server named **PostgreLocal**. It contains a database named **Arc-Demo-PG** which has 4 tables.
...
## After this exercise, you have learned the following,
   ...
   - How to migrate from different Multi-cloud server.
