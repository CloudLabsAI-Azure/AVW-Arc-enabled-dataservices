...
In this exercise, you will get familiar with the existing Kubernetes cluster and connect to the data controller and verify the status of the Azure Arc data controller. You will be deploying Log analytic workspace in the azure portal to see the azure arc resources logs. 
...
1. When prompted to enter the Namespace, copy the value of Arc DC Name from the Environment details page. After executing the command, copy the data controller service API endpoint URL value from the output to a **Notepad**.
